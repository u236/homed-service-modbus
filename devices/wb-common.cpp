...
QByteArray WirenBoard::WBUps::pollRequest(void)
{
    switch (m_sequence)
    {
        ...
        case 2 ... 4:
            return m_modbus->makeRequest(m_slaveId, Modbus::ReadInputRegisters, 0x01D0 + (m_sequence - 2) * 0x10, 1);
    }
...
void WirenBoard::WBUps::parseReply(const QByteArray &reply)
{
    switch (m_sequence)
    {
        ...
        case 2 ... 4:
        {
            ...
            if (!m_fullPoll)
                m_endpoints.value(0)->buffer().insert("action", action);
...
QByteArray WirenBoard::WBMwac::pollRequest(void)
{
    switch (m_sequence)
    {
        ...
        case 4 ... 6:
            return m_modbus->makeRequest(m_slaveId, Modbus::ReadInputRegisters, 0x01D0 + (m_sequence - 4) * 0x10, 6);
